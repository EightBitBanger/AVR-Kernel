...
void display_putc(const char ch) {
    ...
    display_address[index] = (uint16_t)ch | ((uint16_t)VGA_CONSOLE_COLOR << 8);
}
...
void display_newline(void) {
    ...
    for (int x = 0; x < display_get_width(); x++) 
        display_address[(display_get_height() - 1) * display_get_width() + x] = (uint16_t)' ' | ((uint16_t)VGA_CONSOLE_COLOR << 8);
}
